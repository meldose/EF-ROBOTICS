...
try:
    ...
    class DummyGPIO:
        ...
        def setup(self,p,_m, pull_up_down=None):       pass
...
def _gpio_init():
    ...
    GPIO.setup(RELAY_PIN_IN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
def _wait_for_gpio_signal(timeout_s: float) -> bool:
    _log(f"[GPIO] Waiting for signal on pin {RELAY_PIN_IN}")
    deadline = time.monotonic() + timeout_s
    while time.monotonic() < deadline:
        if GPIO.input(RELAY_PIN_IN) == GPIO.LOW: # Assuming active low signal
            _log(f"[GPIO] Signal received on pin {RELAY_PIN_IN}")
            return True
        time.sleep(0.1)
    _log(f"[GPIO] Timeout waiting for signal on pin {RELAY_PIN_IN}")
    return False
        ...
        try:
            ...
            while not self._stop.is_set():
                ...
                if self.state == FSMState.ROW_START:
                    ...
                    if self.row_idx > 0:
                        _log(f"[FSM] Row {self.row_idx+1}: gate {ROW_GATE_DWELL_SEC:.0f}s at '{self.waiting['name']}'")
                ...
                #         _log("[FSM] pre-pulse dwell failed → ABORTED"); self.state = FSMState.ABORTED
                    ...
                    _log("[FSM] Post-pulse wait for GPIO signal")
                    if _wait_for_gpio_signal(POST_PULSE_DWELL_S):
                        self.state = FSMState.SUBMIT_B
                    else:
                        _log("[FSM] post-pulse GPIO wait failed → ABORTED"); self.state = FSMState.ABORTED
